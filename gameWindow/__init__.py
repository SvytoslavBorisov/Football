...
from gameSostav import gameSostav
from classModelForTeamsTable import ModelForTableTeams
from classModelForPlayersTable import ModelForTablePlayers, \
    ModelForTablePlayersPass, ModelForTablePlayersGoalkeapers, ModelForTablePlayersSelection
from PyQt5.QtWidgets import QApplication, QMainWindow, \
    QHeaderView, QAbstractItemView, QStyleFactory, qApp, QErrorMessage
...
class Interface(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.listView.setFont(QFont("Courier New", 12))
        ...
        self.modelForTablePlayersPass = ModelForTablePlayersPass(self.ligas[-1].allPlayers,
                                                            func=lambda x: (x.correctPass / x.allPass * 100, x.name)
                                                            if x.skillPass != 1 and x.allPass > 50 else (0, x.name))
        ...
        # Создание модели для таблицы лучших перехватчиков
        self.modelForTablePlayersSelections = ModelForTablePlayersSelection(self.ligas[-1].allPlayers,
                                                            func=lambda x: (x.selections, x.name))

        self.filterModelForTablePlayersSelections = QSortFilterProxyModel()
        self.filterModelForTablePlayersSelections.setSourceModel(self.modelForTablePlayersSelections)
        self.tvSelection.setModel(self.filterModelForTablePlayersSelections)

        self.tvSelection.setSortingEnabled(False)

        self.tvSelection.setFont(QFont("times", 16))
        self.tvSelection.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeToContents)
        self.tvSelection.horizontalHeader().setSectionResizeMode(1, QHeaderView.Stretch)
        self.tvSelection.verticalHeader().setSectionResizeMode(QHeaderView.Stretch)

        # Создание модели для таблицы лучших вратарей
        self.modelForTablePlayersGoalkeapers = ModelForTablePlayersGoalkeapers(self.ligas[-1].allPlayers,
                                                            func=lambda x: (x.saves, x.name) if x.skillPass == 1 else (-1, x.name))

        self.filterModelForTablePlayersGoalkeapers = QSortFilterProxyModel()
        self.filterModelForTablePlayersGoalkeapers.setSourceModel(self.modelForTablePlayersGoalkeapers)
        self.tvGoalKeapers.setModel(self.filterModelForTablePlayersGoalkeapers)

        self.tvGoalKeapers.setSortingEnabled(False)

        self.tvGoalKeapers.setFont(QFont("times", 16))
        self.tvGoalKeapers.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeToContents)
        self.tvGoalKeapers.horizontalHeader().setSectionResizeMode(1, QHeaderView.Stretch)
        self.tvGoalKeapers.verticalHeader().setSectionResizeMode(QHeaderView.Stretch)

        # Обновление таблиц
        self.onlineTables()

        self.bNextTour.clicked.connect(self.playOneTour)
        self.bExit.clicked.connect(self.closeGame)
    ...
    def playOneTour(self):
        ...
        if self.ligas[self.globalNumberSeason].tourNow != (self.ligas[self.globalNumberSeason].countTeams - 1) * 2:
            ...
            if self.ligas[self.globalNumberSeason].tourNow == (self.ligas[self.globalNumberSeason].countTeams - 1) * 2:
                err = QErrorMessage()
                err.showMessage('Следующее нажатие - новый сезон')
                err.exec()
        else:

            with open(f'season{self.globalNumberSeason}.txt', 'w') as f:
                for i in range(self.modelForTableTeams.rowCount()):
                    f.write(self.modelForTableTeams.teams[i].name + ' ' + str(
                        self.modelForTableTeams.teams[i].points) + '\n')
    ...
    def onlineTables(self):
        ...
        # Обновление данных таблиц
        self.modelForTablePlayersGoalkeapers.newData()
        startIndex = self.modelForTablePlayersGoalkeapers.createIndex(0, 0)
        stopIndex = self.modelForTablePlayersGoalkeapers.createIndex(self.modelForTablePlayersGoalkeapers.rowCount(),
                                                              self.modelForTablePlayersGoalkeapers.columnCount())

        self.tvGoalKeapers.dataChanged(startIndex, stopIndex)

        # Обновление данных таблиц
        self.modelForTablePlayersSelections.newData()
        startIndex = self.modelForTablePlayersSelections.createIndex(0, 0)
        stopIndex = self.modelForTablePlayersSelections.createIndex(self.modelForTablePlayersSelections.rowCount(),
                                                              self.modelForTablePlayersSelections.columnCount())

        self.tvSelection.dataChanged(startIndex, stopIndex)
        ...
        if stTempClick0 == 'Tур':
            if row + 1 == self.model.rowCount():
                for i in range(self.ligas[-1].countTeams // 2):
                    team1 = self.ligas[-1].findGameForIds(self.ligas[-1].games[int(stTempClick1) - 1][i].ids).teams[
                        0].name
                    result = self.ligas[-1].findGameForIds(
                        self.ligas[-1].games[int(stTempClick1) - 1][i].ids).returnResult()
                    team2 = self.ligas[-1].findGameForIds(self.ligas[-1].games[int(stTempClick1) - 1][i].ids).teams[
                        1].name
